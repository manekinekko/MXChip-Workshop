...
function getTwin() {
  ...
  var url = `${this.functionAppName}?action=get&t=${timespan}`;
  this.$http
    .jsonp(url)
    .then(function(data) {
      return data.json();
    ...
    .then(function(data) {
      ...
      if (scope.reportedTwin.$metadata) {
        scope.lastUpdated = scope.reportedTwin.$metadata.$lastUpdated;
      }

      scope.lastUpdated = new Date(lastUpdated)
...
function updateUserLEDState() {
  ...
  this.stateUpdateEndpoint = `${
    this.functionAppName
  }?action=set&state=${state}&key=userLED&t=${timespan}`;
...
function updateRgbLEDState() {
  ...
  this.stateUpdateEndpoint = `${
    this.functionAppName
  }?action=set&state=${state}&key=rgbLED&t=${timespan}`;
  setTimeout(this.getTwin, 1000);
  if (
    state === 1 &&
    this.rgbColor.r === 0 &&
...
function updateRgbLEDColor() {
  ...
  this.stateUpdateEndpoint = `${this.functionAppName}?action=set&state=${
    state.r
  },${state.g},${state.b}&key=rgbLEDColor&t=${timespan}`;
  setTimeout(this.getTwin, 1000);
}
